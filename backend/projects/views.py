...
class TaskListCreateView(APIView):
    def get(self, request, project_id):
        ...
        if q:
            ...
            .filter(Q(title__icontains=q) | Q(description__icontains=q)).select_related('assignee').order_by('position'))
        else:
            tasks = (Task.objects.filter(project_id=project_id).select_related('assignee').order_by('status', 'position'))
